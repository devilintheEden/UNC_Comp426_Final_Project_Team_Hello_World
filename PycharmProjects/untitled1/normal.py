...
def h_normal(Hospital_length_of_stay_mean, Hospital_length_of_stay_std, Hospital_initial_condition, rate,
    ...
    while t <= TotalTimeLength:
        ...
        mt_temp = initial_condition * probability_still_in_system + ans  # mean
        mt.append(mt_temp)
    ...
    data = {
        "# Patients": ["25", "50", "75", "100", "125", "150", "175"],
...
@streamlit.cache(suppress_st_warning=True)
def icu_normal(ICU_length_of_stay_mean, ICU_length_of_stay_std, ICU_initial_condition, rate, Percentage_icu):
...
@streamlit.cache(suppress_st_warning=True)
def ed_normal(ED_length_of_stay_mean, ED_length_of_stay_std, ED_initial_condition, Daily_arrival_rate, Hourly_pattern):
...
st.subheader("normal")
            tlist_, mt_, mt5_, mt95_, df_ = another.ed_normal(ed_mean, ed_std, ed_initial, r_daily, r_hourly)

            dfr_ = pd.DataFrame(list(zip(mt_, mt5_, mt95_)),
                                columns=["Mean", "Lower", "Upper"],
                                index=pd.RangeIndex(len(tlist), name="x"))

            line_ = alt.Chart(dfr_.reset_index()).mark_line(color="#FF9999").encode(
                x="x",
                y="Mean",
            )

            band_ = line_.mark_area(opacity=0.5, color="#CCFFCC").encode(
                x='x',
                y='Lower',
                y2='Upper'
            )
            band_.encoding.x.title = "Days"
            band_.encoding.y.title = "Census"
            line_ + band_
            st.write(df_)
...
st.subheader("Hospital normal")
            tlist_, mt_, mt5_, mt95_, df_ = another.h_normal(h_mean, h_std, h_initial, rate, p_hos)
...
st.subheader("ICU normal")
tlist_, mt_, mt5_, mt95_, df_ = another.icu_normal(icu_mean, icu_std, icu_initial, r_daily, p_icu)

dfr_ = pd.DataFrame(list(zip(mt_, mt5_, mt95_)),
                    columns=["Mean", "Lower", "Upper"],
                    index=pd.RangeIndex(len(tlist_), name="x"))
line_ = alt.Chart(dfr_.reset_index()).mark_line(color="#FF9999").encode(
    x="x",
    y="Mean",
)
