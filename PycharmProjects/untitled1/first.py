...
img = Image.open(r"/Users/yunyingzhu/Downloads/logo.png")
st.image(img)
st.title('COVID-CAT')
...
if st.checkbox("Click here to see methodology", key="M"):
    ...
    img2 = Image.open(r"/Users/yunyingzhu/Downloads/Patient-Flow.png")
...
# if st.sidebar.checkbox("SIR"):
# st.sidebar.subheader("SIR")
# current_hospitalized = float(st.sidebar.text_input("Current Hospitalized Patients", value=50))
# doubling_time = float(st.sidebar.text_input("Doubling Time", value=8))
# hospitalized_rate = float(st.sidebar.text_input("Hospitalized Rate (%)", value=0.1))
# infectious_days = float(st.sidebar.text_input("Infectious Days", value=10))
# market_share = float(st.sidebar.text_input("Hospital Market Share (%)", value=0.3))
...
# mitigation_date = st.sidebar.text_input("Mitigation Date", value=datetime.date(2020, 10, 8))
# relative_contact_rate = float(st.sidebar.text_input("Relative Contact Rate (%)", value=0.5))
# arriving_rate = float(st.sidebar.text_input("Arriving Rate", value=0.5))
...
if st.sidebar.checkbox("Click here to display results", key="B"):
    if h_mean != 0 and h_std != 0:  # and r_daily is not None and p_hos != 0:
        with st.spinner("Running .. progress shown here"):
            st.subheader("Hospital real value")
