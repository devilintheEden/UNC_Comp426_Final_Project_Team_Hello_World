...
def generate_blank_pdf_sample(usr_id,w,h):
    w_offset = h_offset = 0
    ...
    while k < len(ALL_CHARS):
        ...
        for j in range(grids_num_h):
            ...
            if(k >= len(ALL_CHARS)):
                break
        images.append(img)
    file_path = os.path.join(BASE_PATH, "Users", str(usr_id), "Blank")
    helper_create_path(file_path)
    print(file_path)
    images[0].save(os.path.join(file_path, 'Sample_Latin_Alphabets_' + str(w) + '_' + str(h) + '.pdf'), "PDF" ,resolution=100.0, save_all=True,append_images=images[1:])

def create_svg_from_pdf(usr_id,project_name,upload_file_name,w,h):
    upload_folder_path = os.path.join(BASE_PATH,"Users", str(usr_id), 'Projects', project_name,'Uploads')
    ...
    if(w < h):
        ...
        h_offset = int((grid_edge - grid_edge / w * h) / 2)
    images = convert_from_bytes(open(upload_pdf_path, 'rb').read(),size=(paper_width, paper_height))
    k = 0
    for image in images:
        for j in range(grids_num_h):
            for i in range(grids_num_w):
                if(k >= len(ALL_CHARS)):
                    break
                left_up_corner = (left_padding + i * (grid_edge + grid_interval), upper_padding + j * (grid_edge + grid_interval))
                cropped_image = image.crop((left_up_corner[0] + w_offset + 4, left_up_corner[1] + grid_interval + h_offset + 4, left_up_corner[0] + grid_edge - w_offset, left_up_corner[1] + grid_edge + grid_interval - h_offset))
                ...
                subprocess.run(["potrace", "-s", os.path.join(save_file_path, str(k) +'.bmp'), "-o", os.path.join(save_file_path, str(k) +'.svg')])
                subprocess.run(["rm", os.path.join(save_file_path, str(k) +'.bmp')])
                k = k + 1
            if(k >= len(ALL_CHARS)):
                break

def generate_TTF_from_SVG(usr_id,project_name,font_name):
    file_path = os.path.join(BASE_PATH,"Users", str(usr_id), 'Projects', project_name,'Uploads')
    ...
    for i in range(len(ALL_CHARS)):
        ...
        new_font.paste()
        glyph.width = 1000
        glyph.importOutlines(os.path.join(file_path, 'Images_From_PDF', str(i) + '.svg'))
    ...
    output_path = os.path.join(BASE_PATH,"Users", str(usr_id), 'Projects', project_name, 'Output')
...
if __name__ == '__main__':
    ...
    if(arguments[0] == '0'):
        # mode=0,usr_id,grid_width,grid_height
        generate_blank_pdf_sample(int(arguments[1]), int(arguments[2]), int(arguments[3]))
    elif(arguments[0] == '1'):
        # mode=1,usr_id,project_name,upload_file_name,grid_width,grid_height
        create_svg_from_pdf(int(arguments[1]), arguments[2], arguments[3], int(arguments[4]), int(arguments[5]))
    elif(arguments[0] == '2'):
        # mode=1,usr_id,project_name,font_name
